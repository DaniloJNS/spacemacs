...
- [[#install][Install]]
  ...
  - [[#adding-new-dictionaries-to-hunspell][Adding new dictionaries to hunspell]]
...
** Adding new dictionaries to hunspell
Access the libreoffice dictionary repository via the link https://cgit.freedesktop.org/libreoffice/dictionaries/tree, look for the directory of the language you are looking for on this page and enter it.
look for two files one with suffix .aff and .dicc click on plain on the left side and copy the link in your browser's search bar. And use the following ~wget~ command to download both.
You should place them in one of ~hunspell~'s search directories, you can get the list of these directories by the command ~hunspell -D~. Create a new directory for your language within the directory
with the same name where it was stored in the libreoffice repository. Example added a dictionary for Brazilian Portuguese.
#+begin_src shell
  mkdir -p ~/Library/Spelling
  cd ~/Library/Spelling
  mkdir pt_BR && cd pt_BR
...
OR try install via AUR for Arch Linux, by example, the grammar via freedesktop for pt BR is broken. In this case, was used ~hunspell-pt-br~.
